...
class Client:
    ...
    def check_balance_interface(self, token_address, min_value) -> bool:
        logger.info(f'{self.address} | balanceOf | check balance of {token_address}')
        ...
        if balance < min_value * 10 ** decimal:
            logger.error(f'{self.address} | balanceOf | not enough {token_address}')
            return False
        return True
    ...
    def verif_tx(self, tx_hash) -> bool:
        try:
            logger.info(f'{self.address} | wait for transaction receipt: {tx_hash.hex()}')
            data = self.w3.eth.wait_for_transaction_receipt(tx_hash, timeout=500)
            if 'status' in data and data['status'] == 1:
                logger.success(f'{self.address} | transaction was successful: {self.network.explorer}tx/{tx_hash.hex()}')
                return True
            else:
    ...
    def approve_interface(self, token_address: str, spender: str, amount: Optional[TokenAmount] = None) -> bool:
        ...
        if amount.Wei <= approved.Wei:
            logger.success(f'{self.address} | approve | already approved')
            return True
    ...
    def get_eth_price(self, token='ETH'):
        ...
        if response.status_code != 200:
            logger.error(f'code: {response.status_code} | json: {response.json()}')
            return None
